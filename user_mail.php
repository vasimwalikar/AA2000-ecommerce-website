...
$row = mysql_fetch_array($query);
	$query1=mysql_query("select * from customers where CustomerID='$ses_id'");
...
if(isset($_POST['submit'])){
	...
	mysql_query("insert into message (ID,CustomerID,Name,Email,Subject,Message,Date_created) values ('','$ses_id','$Name','$Email','$Subject','$Message','$new')") or die (mysql_error());  
    mysql_query("insert into sent_messages (ID,CustomerID,Name,Email,Subject,Message,Date_created) values ('','$ses_id','$Name','$Email','$Subject','$Message','$new')") or die (mysql_error());  
?>
<script>
alert("Message Sent");
...
<form action="delete_inbox.php" method="post">
      ...
      <?php
	   if($inbox !="0")
	   $data=mysql_query("select * from reply_message where CustomerID='$inbox'");
...
<br/><strong>Click All: <input type="checkbox" onClick="toggle(this)" /></strong>
       <?php 
       $query2=mysql_query("select * from sent_messages where CustomerID='$id'");
